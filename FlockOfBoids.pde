...
void setup() {
  ...
  for (int i = 0; i < initBoidNum; i++)
    flock.add(new Boid(new Vector(flockWidth / 2, flockHeight / 2, flockDepth / 2),representation, rendering));
}

void draw() {
  int minutoNuevo = minute();
  int segundoNuevo = second();
  if ((minutoNuevo >= minutoActual+1) && (segundoNuevo==segundoActual)) {
    exit();
